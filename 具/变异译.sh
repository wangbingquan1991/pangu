...
#
# 又：变量名一律用 ASCII。非为媚 bash，而是**此具曾因此坏过**——见上「果=」。
#
# 同一病发了第二回：后来添的「验数之检」又写了 `验数=`／`实跑=`。
# 其故不只是 bash：**zsh 于 C/POSIX 之地亦不认汉字是标识符之字符**
# （LC_CTYPE 判之），而定时、CI、管道里跑皆是 C 之地；
# 脚本内 `export LC_ALL=…UTF-8` 救不得——解析表在启动之际已定死。
# 报的是 `command not found: 验数=141`，而病根不在那一行，在环境里。
#
# 规矩写在注里，注是会朽的；故下以机器守之（「汉字之名之检」）。
# ─────────────────────────────────────────────────────────────
set -u

# ---- 汉字之名之检：注里的规矩，须有机器守着 ----
#
# 此式捕「赋值之左有汉字」者，不误捕 `SRC=译.go`（汉字在值里）、注行（首字 #）。
# 其详与其验，见 具/变异枢.sh 同检之注。
if HANNAME=$(grep -nE '^[[:space:]]*[A-Za-z_]*[一-鿿][A-Za-z_一-鿿]*=' "$0" 2>/dev/null); then
	print -r -- "【汉字之名之检】不过：本具竟以汉字为变量之名——"
	print -r -- "$HANNAME" | sed 's/^/            /'
	print -r -- "            zsh 于 C/POSIX 之地不认汉字是标识符之字符，"
	print -r -- "            则「汉字 = 值」不是赋值而是命令，报 command not found。"
	print -r -- "            一律改用 ASCII 之名（值里、串里用汉字无妨）。止。"
	exit 1
fi
...
#
# 单子写就的那日，它是全的；第二日便不是了。
# 变异十六（丙型之址为零者不书）初跑报「存活」，
# 而 Test规范式之全 明明专为杀它而立——**只因它不在单子上，从未被跑**。
#
# 此是同一病之第三发。前两发是 器号表 与 助记码表，
# 其治法是「双向之约」：不抄，径由 卦名/分类 导出。
# 此发之治法同：**不抄，便无从漏**。整包之测不过六秒，全跑即是。
#
# 教训：一处知识有两份拷贝，其中一份必朽；
#       而朽的那份若是个「过滤器」，它漏掉的东西**不报错，只是不见了**。
...
# 数一数：包内实有之验，与实跑之验。**二者当同。**
#
# 变异十六初跑报「存活」，而 Test规范式之全 明明专为杀它而立——
# 只因它不在那张手抄的 -run 名单上，**从未被跑**。
# 名单已废（见 run 之上），然废了名单不等于此类事不会再有：
# 一验若因建不起来、因名不合、因文件未入包而未跑，
# 它也是**静默地不见了**。故立此数，教此事看得见。
TESTS=$(grep -hoE '^func Test[^ (]+' *_test.go | wc -l | tr -d ' ')
RAN=$(./跑.sh test -count=1 -v ./... 2>&1 | grep -cE '^--- (PASS|FAIL|SKIP)')
if [[ "$TESTS" -ne "$RAN" ]]; then
	print -r -- "【验数之检】⚠ 包内实有 $TESTS 验，实跑 $RAN 验——有验未被跑。"
	print -r -- "            未被跑之验杀不着任何变异，而此具**不会报**——它只报「存活」。"
else
	print -r -- "【验数之检】$RAN 验皆跑 ✓"
fi
print -r -- ""
